...
async function saveStore(store: CrmStore) {
  ...
  if (supabaseEnabled && supabase) {
        const { data: existing } = await supabase
          ...
          .eq("id", stateId)
          .maybeSingle();
        const merged = { ...(existing?.data ?? {}), ...nextStore };
        const { error } = await supabase
          .from("crm_app_state")
          .upsert({ id: stateId, data: merged, updated_at: new Date().toISOString() });
...
function buildAwardeeRecord(data: Partial<Awardee>, fallbackIndex: number): Awardee {
    const id = data.id?.trim() || `HU-2026-${String(fallbackIndex + 1001).padStart(4, "0")}`;
    return {
          id,
...
export async function listAwardees(): Promise<Awardee[]> {
    const db = requireSupabase();
    const { data, error } = await db.from("awardees").select("*").order("createdAt", { ascending: false });
    if (error) throw error;
    return (data ?? []) as unknown as Awardee[];
}

export async function getAwardee(id: string): Promise<Awardee | undefined> {
    const db = requireSupabase();
    const { data, error } = await db.from("awardees").select("*").eq("id", id).maybeSingle();
    if (error) throw error;
    return (data ?? undefined) as unknown as Awardee | undefined;
}

export async function createAwardee(data: Partial<Awardee>): Promise<Awardee> {
    const db = requireSupabase();
    const { count } = await db.from("awardees").select("id", { count: "exact", head: true });
    const awardee = buildAwardeeRecord(data, count ?? 0);
    const { data: inserted, error } = await db.from("awardees").insert(awardee).select().single();
    if (error) throw error;
    return inserted as unknown as Awardee;
}

export async function updateAwardee(id: string, patch: Partial<Awardee>): Promise<Awardee | null> {
    const db = requireSupabase();
    const { data, error } = await db.from("awardees").update(patch).eq("id", id).select().maybeSingle();
    if (error) throw error;
    return (data ?? null) as unknown as Awardee | null;
}

export async function deleteAwardee(id: string): Promise<boolean> {
    const db = requireSupabase();
    const { error, count } = await db.from("awardees").delete({ count: "exact" }).eq("id", id);
    if (error) throw error;
    return (count ?? 0) > 0;
}

export type CheckInResult =
    | { ok: true; awardee: Awardee }
  | { ok: false; reason: "not_found" }
  | { ok: false; reason: "already_registered"; awardee: Awardee };

export async function checkInAwardee(id: string, patch: Partial<Awardee>): Promise<CheckInResult> {
    const db = requireSupabase();
    const { data, error } = await db
      .from("awardees")
      .update(patch)
      .eq("id", id)
      .neq("registrationStatus", "Registered")
      .select()
      .maybeSingle();
    if (error) throw error;
    if (data) return { ok: true, awardee: data as unknown as Awardee };
    const current = await getAwardee(id);
    if (!current) return { ok: false, reason: "not_found" };
    return { ok: false, reason: "already_registered", awardee: current };
}

export type IssueCertificateResult =
    | { ok: true; awardee: Awardee }
  | { ok: false; reason: "not_found" }
  | { ok: false; reason: "not_registered"; awardee: Awardee }
  | { ok: false; reason: "already_issued"; awardee: Awardee };

export async function issueCertificate(id: string, patch: Partial<Awardee>): Promise<IssueCertificateResult> {
    const db = requireSupabase();
    const current = await getAwardee(id);
    if (!current) return { ok: false, reason: "not_found" };
    if (current.registrationStatus !== "Registered") return { ok: false, reason: "not_registered", awardee: current };
    if (current.certificateStatus === "Issued") return { ok: false, reason: "already_issued", awardee: current };
    const { data, error } = await db
      .from("awardees")
      .update(patch)
      .eq("id", id)
      .neq("certificateStatus", "Issued")
      .select()
      .maybeSingle();
    if (error) throw error;
    if (!data) {
          const latest = await getAwardee(id);
          return { ok: false, reason: "already_issued", awardee: latest ?? current };
    }
    return { ok: true, awardee: data as unknown as Awardee };
}
...
export async function importAwardees(rows: Array<Record<string, unknown>>) {
    const db = requireSupabase();
    const { data: existing, error: fetchError } = await db.from("awardees").select("id,email,phone");
    if (fetchError) throw fetchError;
    const existingRows = (existing ?? []) as unknown as Array<{ id: string; email: string; phone: string }>;
    const seenIds = new Set(existingRows.map((a) => a.id.toLowerCase()));
    const seenEmails = new Set(existingRows.map((a) => (a.email || "").toLowerCase()).filter(Boolean));
    const seenPhones = new Set(existingRows.map((a) => (a.phone || "").replace(/\D/g, "")).filter(Boolean));
    const { count } = await db.from("awardees").select("id", { count: "exact", head: true });
  ...
  rows.forEach((row, index) => {
        const awardee = normalizeImportRow(row, baseIndex + index);
  ...
  if (imported.length) {
        const { error: insertError } = await db.from("awardees").insert(imported);
