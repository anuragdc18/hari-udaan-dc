...
const app = new Hono()
  ...
  .delete('/awardees/:id', async (c) => {
        const deleted = await deleteAwardee(c.req.param('id'));
        if (!deleted) return c.json({ error: 'Awardee not found' }, 404);
  ...
  .post('/registration/:id/check-in', async (c) => {
        const body = await c.req.json().catch(() => ({}));
        const result = await checkInAwardee(c.req.param('id'), {
                ...body,
        ...
        if (!result.ok) {
                if (result.reason === 'not_found') return c.json({ error: 'Awardee not found' }, 404);
                return c.json({
                          error: `Already registered by ${result.awardee.checkedInBy || 'another user'}`,
                          awardee: result.awardee,
                }, 409);
        }
        return c.json({ awardee: result.awardee }, 200);
  })
  ...
  .post('/certificates/:id/issue', async (c) => {
        const body = await c.req.json().catch(() => ({}));
        const result = await issueCertificate(c.req.param('id'), {
        ...
        if (!result.ok) {
                ...
                return c.json({
                          error: `Certificate already issued by ${result.awardee.certificateIssuedBy || 'another user'}`,
                          awardee: result.awardee,
                }, 409);
        }
        return c.json({ awardee: result.awardee }, 200);
